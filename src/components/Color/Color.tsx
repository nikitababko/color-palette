...
import { CircleAdd, ColorBar, ColorContainer, ColorPickerWrapper, ColorText } from './styles';
...
export const Color: React.FC<ColorProps> = ({
  ...
  const colorPickerWrapperReference = useRef<HTMLDivElement>(null);

  useOutsideClick([colorNameReference, colorPickerWrapperReference], () =>
  ...
  useEffect(() => {
    if (!isOpenedColorPicker && color.id === colorProperty.id) {
      setColors((previousState) =>
  ...
  }, [color.hex, color.id, colorProperty.id, isOpenedColorPicker, setColors]);
  ...
  return (
    ...
    >
      {!isMaxColorCount && (
        <React.Fragment>
          {!isFirstElement && (
            <ColorBar top={0} right="auto" bottom="auto" left={0}>
              <CircleAdd top="-23px" left="-23px" onClick={handleAddColor}>
                <PlusIcon />
              </CircleAdd>
            </ColorBar>
          )}
          ...
          {!isLastElement && (
            <ColorBar top="auto" right={0} bottom={0} left="auto">
              <CircleAdd top="23px" left="23px" onClick={handleAddColor}>
                <PlusIcon />
              </CircleAdd>
            </ColorBar>
      ...
      {isOpenedColorPicker && (
        <ColorPickerWrapper ref={colorPickerWrapperReference} data-testid="ColorPickerWrapper">
          <ColorPicker
            color={color?.hex}
            onChange={({ hex }) => setColor((previousState) => ({ ...previousState, hex }))}
          />
        </ColorPickerWrapper>
      )}
    </ColorContainer>
